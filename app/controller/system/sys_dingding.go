...
import (
	...
	systemModel "github.com/championlong/go-quick-start/app/model/system"
...
func (ding *DingRoles) DingApplyRoles(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&applyRoles); err != nil {
		global.GVA_LOG.Error("钉钉申请请求解析失败", zap.Error(err))
	}
	signature := c.Query("signature")
	timestamp := c.Query("timestamp")
	nonce := c.Query("nonce")

	callbackCrypto := system.GetDingTalkCrypto()
	decryptMsg, err := callbackCrypto.GetDecryptMsg(signature, timestamp, nonce, applyRoles.Encrypt)
	if err != nil {
		global.GVA_LOG.Error("钉钉审批解密请求信息失败", zap.Error(err))
	}
	msg, err := callbackCrypto.GetEncryptMsg("success")
	if err != nil {
		global.GVA_LOG.Error("钉钉审批获取加密信息失败", zap.Error(err))
	}
	c.JSON(200, msg)
	err = json.Unmarshal([]byte(decryptMsg), &eventSubscriptionMsg)
	if err != nil {
		global.GVA_LOG.Error("钉钉审批序列化审批信息失败", zap.Error(err))
	}
	...
	global.GVA_LOG.Info(fmt.Sprintf("%s 钉钉审批信息 %s", ctx2, decryptMsg))
	approveInfoResponse, err := system.GetDingApproveInfo(eventSubscriptionMsg.ProcessInstanceId)
	if err != nil {
		global.GVA_LOG.Error("获取钉钉详情审批信息失败", zap.Error(err))
